...
class Mammotion extends utils.Adapter {
    ...
    async buildRouteSettingsFromPayload(deviceKey, payload) {
        ...
        return {
            ...
            channelWidthCm: Math.min(ROUTE_CHANNEL_WIDTH_MAX_CM, Math.max(ROUTE_CHANNEL_WIDTH_MIN_CM, Math.trunc(payload.channelWidthCm ?? payload.routeChannelWidthCm ?? base.channelWidthCm))),
    ...
    async ensureDeviceStateObjects(channelId) {
        ...
        const normalizedRouteWidth = Math.min(ROUTE_CHANNEL_WIDTH_MAX_CM, Math.max(ROUTE_CHANNEL_WIDTH_MIN_CM, Math.trunc(Number(routeWidthState?.val))));
